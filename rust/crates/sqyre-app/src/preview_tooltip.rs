...
fn draw_rect_outline(
    ...
    // Inclusive max edge like Go's rectangle stroke on image coords.
    let x1 = rx - 1;
    let y1 = by - 1;
...
fn draw_point_marker(img: &mut RgbaImage, cx: i32, cy: i32, c: Rgba<u8>, thick: i32) {
    // Circle radius 8 (approx) + crosshair arms ±15 — mirrors Go DrawPreviewPointMarker.
    for dy in -8..=8 {
        for dx in -8..=8 {
