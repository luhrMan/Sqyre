//! Framed tooltip sections with wrapping rows (Go `wrapTooltipSection` + `rowWrapLayout`).
...
/// Sqyre-framed section (Go `wrapTooltipSection` / `WrapSqyreFrame`).
...
/// Framed section whose children flow left-to-right and wrap (Go pill row / `RowWrapLayout`).
///
