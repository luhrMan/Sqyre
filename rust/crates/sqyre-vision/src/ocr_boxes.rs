//! Word-box target location (Go `findTargetInBoxes` / `textFromOCRBoxes`).
...
/// Join non-empty word boxes with spaces (Go `textFromOCRBoxes`).
...
/// Center of the union of boxes whose words match `target` (Go `findTargetInBoxes`).
