...
func TestProgramRepo_LoadFromConfig(t *testing.T) {
	...
	program, err := repo.Get("testprogram")
	if err != nil {
		t.Fatalf("Failed to get testprogram: %v", err)
	}
...
func TestProgramRepo_DecodeNestedStructures(t *testing.T) {
	...
	testItem, exists := program.Items["testitem"]
	if !exists {
		t.Fatal("testitem should exist in Items")
	}
	...
	testPoint, exists := coords.Points["testpoint"]
	if !exists {
		t.Fatal("testpoint should exist in Points")
	}
	...
	testArea, exists := coords.SearchAreas["testarea"]
	if !exists {
...
func TestProgramRepo_CRUD(t *testing.T) {
	...
	newProgram := repo.New()
	newProgram.Name = "New Test Program"

	// Test Set (skip actual save to avoid file I/O issues)
	repo.mu.Lock()
	repo.models["newprogram"] = newProgram
	repo.mu.Unlock()

	// Test Get
	retrieved, err := repo.Get("newprogram")
	if err != nil {
		t.Fatalf("Failed to get new program: %v", err)
	}

	if retrieved.Name != "New Test Program" {
		t.Errorf("Expected name 'New Test Program', got '%s'", retrieved.Name)
	}
	...
	repo.mu.Lock()
	delete(repo.models, "newprogram")
	repo.mu.Unlock()

	_, err = repo.Get("newprogram")
...
func TestProgramRepo_ItemsAggregateAccess(t *testing.T) {
	...
	item, exists := program.Items["testitem"]
	if !exists {
...
func TestProgramRepo_CoordinatesAggregateAccess(t *testing.T) {
	...
	point, exists := coords.Points["testpoint"]
	if !exists {
