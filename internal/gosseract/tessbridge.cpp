...
// InitFromMemory initializes Tesseract with traineddata from a buffer (no disk write).
// Uses TessBaseAPI::Init(data, data_size, language, ...) available in Tesseract 4.x+.
int InitFromMemory(TessBaseAPI a, unsigned char* data, int size, char* lang, char* errbuf) {
    tesseract::TessBaseAPI* api = (tesseract::TessBaseAPI*)a;
    if (data == nullptr || size <= 0 || lang == nullptr) {
        return -1;
    }
    fflush(stderr);
    ...
    int ret = api->Init((const char*)data, size, lang, tesseract::OEM_DEFAULT, nullptr, 0, nullptr, nullptr, false, nullptr);
    (void)freopen("/dev/null", "a", stderr);
    dup2(original_stderr, STDERR_FILENO);
    close(original_stderr);
    setbuf(stderr, NULL);
