...
type Client struct {
	...
	// TessdataFromMemory, if non-nil and non-empty, is used to init from RAM (no disk write).
	// When set, Init uses TessBaseAPI::Init(data, size, language) instead of a path.
	TessdataFromMemory []byte
...
func (client *Client) SetTessdataPrefix(prefix string) error {
	...
	client.TessdataFromMemory = nil
	client.flagForInit()
	return nil
}

// SetTessdataFromMemory sets traineddata from a byte slice so Init runs entirely in RAM (no disk write).
// Preferred on Windows to avoid path/permission issues. Clears TessdataPrefix when set.
func (client *Client) SetTessdataFromMemory(data []byte) error {
	if len(data) == 0 {
		return fmt.Errorf("tessdata from memory cannot be empty")
	}
	client.TessdataFromMemory = data
	client.TessdataPrefix = ""
	client.flagForInit()
	return nil
}
