...
docker build -f .devcontainer/builds/android/opencv/Dockerfile.opencv-android \
  ...
  .devcontainer/builds/android/opencv
```

Build time is long (download OpenCV + contrib, then compile for four ABIs). To build only one ABI:
...
- Per-ABI: `/tmp/opencv-android-build/install_<abi>/` (libs + headers).
- SDK-style: `/tmp/opencv-android-build/opencv-android-sdk/native/libs/<abi>/` and `.../jni/include/`.

Copy them out:

```bash
docker create --name opencv-android opencv-android:local
docker cp opencv-android:/tmp/opencv-android-build/opencv-android-sdk ./opencv-android-sdk
...
```bash
.devcontainer/builds/android/opencv/build-opencv-android.sh
```
...
| `BUILD_ROOT` | `/tmp/opencv-android-build` | Source and build directory |
